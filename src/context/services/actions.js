...
const getAll = (params) => {
  ...
  return async (dispatch, getState) => {
    console.log('ACTIONS::SERVICES::getAll', params);
    dispatch(request());
    const response = await services.getAll(params, getState().users.auth.token)
    console.log('ACTIONS::SERVICES::getAll::RESPONSE FROM SERVIVE', response);
    if (response.success) {
...
const get = (id) => {
  console.log('ACTIONS::SERVICES::get', id);
  ...
  return async (dispatch, getState) => {
    ...
    console.log('ACTIONS::SERVICES::get::RESPONSE FROM SERVIVE', response);
    if (response.success) {
      dispatch(success(response.data));
    } else {
      // if (response.error && response.error.code === 'ERROR_401') {
...
const saveOrUpdate = (service) => {
  ...
  return async (dispatch, getState) => {
    console.log('ACTIONS::SERVICES::saveOrUpdate', service);
    dispatch(request(service));

    let response;
    if (service.id) {  // editing a existing record
      response = await services.update(service, getState().users.auth.token)
    } else {
      response = await services.save(service, getState().users.auth.token)
    }
    console.log('ACTIONS::SERVICES::saveOrUpdate::RESPONSE FROM SERVIVE', response);
    if (response.success) {
      dispatch(success(response.data));
    } else {
      // if (response.error && response.error.code === 'ERROR_401') {
...
const del = (id) => {
  console.log('ACTIONS::SERVICES::del', id);
  ...
  return async (dispatch, getState) => {
    ...
    console.log('ACTIONS::SERVICES::del::RESPONSE FROM SERVIVE', response);
    if (response.success) {
      dispatch(success(id));
    } else {
      // if (response.error && response.error.code === 'ERROR_401') {
